...
def surface_distance(target, estimated, spacing=[1, 1, 3]):
    ...
    distances_a, _ = nbrs_a.kneighbors(b_bound) if nbrs_a and b_bound.size > 0 else np.inf
    distances_b, _ = nbrs_b.kneighbors(a_bound) if nbrs_b and a_bound.size > 0 else np.inf
