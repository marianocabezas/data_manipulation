...
def analysis_by_sizes(target, estimated, sizes):
    ...
    fpf_list = [100.0 * fp_i / (fp_i+tp_i) if fp_i+tp_i > 0 else np.nan
                for tp_i, fp_i in zip(tpd_list, fpd_list)]
    dscd_list = [2.0 * tp_i / (tp_i + fp_i + gt_i) if (tp_i + fp_i + gt_i) > 0 else np.nan
                 for tp_i, fp_i, gt_i in zip(tpd_list, fpd_list, gtd_list)]
    dscs_list = [2.0 * tp_i / (tp_i + fp_i + gt_i) if (tp_i + fp_i + gt_i) > 0 else np.nan
...
def fp_fraction_seg(target, estimated):
    ...
    fpf = 100.0 * false_positive_seg(target, estimated) / b if b > 0 else np.nan
    return fpf
